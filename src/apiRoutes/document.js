...
import { hocuspocusServer } from '../socket.js';
...
// ========== 添加文档成员 ==========
router.post('/documents/:id/members', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
...
// ========== 移除文档成员 ==========
router.delete('/documents/:id/members/:userId', authenticateToken, async (req, res) => {
  try {
...
// ========== 获取文档成员列表 ==========
router.get('/documents/:id/members', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    const userId = req.user.userId;
