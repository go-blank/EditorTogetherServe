...
lib0_decoding = __toESM(lib0_decoding);
...
let crossws_adapters_node = require("crossws/adapters/node");
crossws_adapters_node = __toESM(crossws_adapters_node);
...
var IncomingMessage = class {
	constructor(input) {
		...
		this.decoder = (0, lib0_decoding.createDecoder)(input);
	}
	get encoder() {
		if (!this.encoderInternal) this.encoderInternal = (0, lib0_encoding.createEncoder)();
	...
	peekVarUint8Array() {
		...
		const result = (0, lib0_decoding.readVarUint8Array)(this.decoder);
	...
	writeVarUint(type) {
		(0, lib0_encoding.writeVarUint)(this.encoder, type);
	}
	writeVarString(string) {
		(0, lib0_encoding.writeVarString)(this.encoder, string);
	}
	get length() {
...
var OutgoingMessage = class {
	constructor(documentName) {
		...
		(0, lib0_encoding.writeVarString)(this.encoder, documentName);
	}
	createSyncMessage() {
		this.type = MessageType.Sync;
		(0, lib0_encoding.writeVarUint)(this.encoder, MessageType.Sync);
		return this;
	}
	createSyncReplyMessage() {
		this.type = MessageType.SyncReply;
		(0, lib0_encoding.writeVarUint)(this.encoder, MessageType.SyncReply);
		return this;
	}
	createAwarenessUpdateMessage(awareness, changedClients) {
		this.type = MessageType.Awareness;
		this.category = "Update";
		const message = (0, y_protocols_awareness.encodeAwarenessUpdate)(awareness, changedClients || Array.from(awareness.getStates().keys()));
		(0, lib0_encoding.writeVarUint)(this.encoder, MessageType.Awareness);
		(0, lib0_encoding.writeVarUint8Array)(this.encoder, message);
		return this;
	}
	writeQueryAwareness() {
		this.type = MessageType.QueryAwareness;
		this.category = "Update";
		(0, lib0_encoding.writeVarUint)(this.encoder, MessageType.QueryAwareness);
		return this;
	}
	writeTokenSyncRequest() {
		this.type = MessageType.Auth;
		this.category = "TokenSync";
		(0, lib0_encoding.writeVarUint)(this.encoder, MessageType.Auth);
		(0, _hocuspocus_common.writeTokenSyncRequest)(this.encoder);
		return this;
	}
	writeAuthenticated(readonly) {
		...
		(0, lib0_encoding.writeVarUint)(this.encoder, MessageType.Auth);
		(0, _hocuspocus_common.writeAuthenticated)(this.encoder, readonly ? "readonly" : "read-write");
		return this;
	}
	writePermissionDenied(reason) {
		...
		(0, lib0_encoding.writeVarUint)(this.encoder, MessageType.Auth);
		(0, _hocuspocus_common.writePermissionDenied)(this.encoder, reason);
		return this;
	}
	writeFirstSyncStepFor(document) {
		this.category = "SyncStep1";
		(0, y_protocols_sync.writeSyncStep1)(this.encoder, document);
		return this;
	}
	writeUpdate(update) {
		this.category = "Update";
		(0, y_protocols_sync.writeUpdate)(this.encoder, update);
		return this;
	}
	writeStateless(payload) {
		this.category = "Stateless";
		(0, lib0_encoding.writeVarUint)(this.encoder, MessageType.Stateless);
		(0, lib0_encoding.writeVarString)(this.encoder, payload);
		return this;
	}
	writeBroadcastStateless(payload) {
		this.category = "Stateless";
		(0, lib0_encoding.writeVarUint)(this.encoder, MessageType.BroadcastStateless);
		(0, lib0_encoding.writeVarString)(this.encoder, payload);
		return this;
	}
	writeSyncStatus(updateSaved) {
		this.category = "SyncStatus";
		(0, lib0_encoding.writeVarUint)(this.encoder, MessageType.SyncStatus);
		(0, lib0_encoding.writeVarUint)(this.encoder, updateSaved ? 1 : 0);
		return this;
	}
	writeCloseMessage(reason) {
		this.type = MessageType.CLOSE;
		(0, lib0_encoding.writeVarUint)(this.encoder, MessageType.CLOSE);
		(0, lib0_encoding.writeVarString)(this.encoder, reason);
		return this;
	}
	toUint8Array() {
		return (0, lib0_encoding.toUint8Array)(this.encoder);
...
var MessageReceiver = class {
	...
	async apply(document, connection, reply) {
		...
		switch (type) {
			...
			case MessageType.Awareness:
				(0, y_protocols_awareness.applyAwarenessUpdate)(document.awareness, message.readVarUint8Array(), connection ?? null);
				break;
			...
			case MessageType.Stateless:
				connection?.callbacks.statelessCallback({
					...
					document,
					payload: (0, lib0_decoding.readVarString)(message.decoder)
				});
				break;
			...
			case MessageType.Auth:
				if (message.readVarUint() === _hocuspocus_common.AuthMessageType.Token) {
	...
	async readSyncMessage(message, document, connection, reply, requestFirstSync = true) {
		...
		switch (type) {
			case y_protocols_sync.messageYjsSyncStep1:
				(0, y_protocols_sync.readSyncStep1)(message.decoder, message.encoder, document);
				...
				break;
			case y_protocols_sync.messageYjsSyncStep2:
				if (connection?.readOnly) {
					const snapshot = yjs.snapshot(document);
					const update = lib0_decoding.readVarUint8Array(message.decoder);
					if (yjs.snapshotContainsUpdate(snapshot, update)) {
				...
				(0, y_protocols_sync.readSyncStep2)(message.decoder, document, connection ? {
					source: "connection",
					connection
				...
				break;
			case y_protocols_sync.messageYjsUpdate:
				if (connection?.readOnly) {
					connection.send(new OutgoingMessage(messageAddress).writeSyncStatus(false).toUint8Array());
					break;
				}
				(0, y_protocols_sync.readUpdate)(message.decoder, document, connection ? {
					source: "connection",
					connection
				} : this.defaultTransactionOrigin ?? { source: "local" });
				if (connection) connection.send(new OutgoingMessage(messageAddress).writeSyncStatus(true).toUint8Array());
...
var Document = class extends yjs.Doc {
	/**
	* Constructor.
	*/
	constructor(name, yDocOptions) {
		...
		this.saveMutex = new async_mutex.Mutex();
		this.lastChangeTime = 0;
		this.name = name;
		this.awareness = new y_protocols_awareness.Awareness(this);
	...
	*/
	merge(documents) {
		(Array.isArray(documents) ? documents : [documents]).forEach((document) => {
			(0, yjs.applyUpdate)(this, (0, yjs.encodeStateAsUpdate)(document));
		});
		return this;
	}
	/**
	...
	*/
	removeConnection(connection) {
		...
		if (entry) (0, y_protocols_awareness.removeAwarenessStates)(this.awareness, Array.from(entry.clients), null);
	...
	*/
	applyAwarenessUpdate(connection, update) {
		(0, y_protocols_awareness.applyAwarenessUpdate)(this.awareness, update, connection);
		return this;
	}
	/**
...
var ClientConnection = class {
	...
	*/
	constructor(websocket, request, documentProvider, hooks, opts, defaultContext = {}) {
		...
		this.socketId = node_crypto.default.randomUUID();
		this.lastMessageReceivedAt = Date.now();
		this.check = () => {
			if (Date.now() - this.lastMessageReceivedAt > this.timeout) this.close(_hocuspocus_common.ConnectionTimeout);
		};
		...
		this.handleQueueingMessage = async (data, rawKey, documentName) => {
			try {
				...
				lib0_decoding.readVarString(tmpMsg.decoder);
				if (!(lib0_decoding.readVarUint(tmpMsg.decoder) === MessageType.Auth && !this.documentConnectionsEstablished.has(rawKey))) {
					this.incomingMessageQueue[rawKey].push(data);
					return;
				}
				this.documentConnectionsEstablished.add(rawKey);
				lib0_decoding.readVarUint(tmpMsg.decoder);
				const token = lib0_decoding.readVarString(tmpMsg.decoder);
				let providerVersion = null;
				if (lib0_decoding.hasContent(tmpMsg.decoder)) providerVersion = lib0_decoding.readVarString(tmpMsg.decoder);
				const sepIdx = rawKey.indexOf("\0");
				...
				try {
					...
					const error = err || _hocuspocus_common.Forbidden;
		...
		this.handleMessage = (data) => {
			...
			try {
				const tmpMsg = new IncomingMessage(data);
				const rawKey = lib0_decoding.readVarString(tmpMsg.decoder);
				const sepIdx = rawKey.indexOf("\0");
				const documentName = sepIdx === -1 ? rawKey : rawKey.substring(0, sepIdx);
		...
		if (this.document) {
			...
			if (this.document.getConnectionsCount() === 0 && !this.document.saveMutex.isLocked()) {
				await this.instance.hooks("onDisconnect", {
					...
					requestParameters: new node_url.URLSearchParams()
				});
...
var Hocuspocus = class {
	...
	closeConnections(documentName) {
		this.documents.forEach((document) => {
			...
			document.connections.forEach((_clients, connection) => {
				connection.close(_hocuspocus_common.ResetConnection);
			});
		});
	}
	/**
	...
	async loadDocument(documentName, request, socketId, connectionConfig, context) {
		...
		try {
			await this.hooks("onLoadDocument", hookPayload, (loadedDocument) => {
				if (loadedDocument instanceof yjs.Doc) (0, yjs.applyUpdate)(document, (0, yjs.encodeStateAsUpdate)(loadedDocument));
				else if (loadedDocument instanceof Uint8Array) (0, yjs.applyUpdate)(document, loadedDocument);
			});
		} catch (e) {
		...
		document.awareness.on("update", (update, origin) => {
			this.hooks("onAwarenessUpdate", {
				...
				states: (0, _hocuspocus_common.awarenessStatesToArray)(document.awareness.getStates())
			});
		});
		return document;
	}
	storeDocumentHooks(document, hookPayload, immediately) {
		...
		return this.debouncer.debounce(debounceId, async () => {
			try {
				...
				if (error instanceof _hocuspocus_common.SkipFurtherHooksError) {
					setTimeout(() => {
	...
	*/
	...
	async openDirectConnection(documentName, context) {
		return new DirectConnection(await this.createDocument(documentName, new Request("http://localhost"), node_crypto.default.randomUUID(), {
...
var Server = class {
	constructor(configuration) {
		...
		this.httpServer = (0, node_http.createServer)(this.requestHandler);
		this.crossws = (0, crossws_adapters_node.default)({
	...
	async listen(port, callback = null) {
		...
		return new Promise((resolve, reject) => {
			this.httpServer.listen({
				...
				if (!this.configuration.quiet && String(process.env.NODE_ENV) !== "testing") this.showStartScreen();
	...
	showStartScreen() {
		...
		console.log();
		console.log(`  ${kleur.default.cyan(`Hocuspocus v${version}${name}`)}${kleur.default.green(" running at:")}`);
		console.log();
		console.log(`  > HTTP: ${kleur.default.cyan(`${this.httpURL}`)}`);
		...
		console.log();
		console.log(`  ${kleur.default.green("Ready.")}`);
		console.log();
	}
};
